...
from xtts_api_server.echo_tts_wrapper import EchoTTSWrapper,supported_languages,InvalidSettingsError
...
# STREAMING VARS (Echo TTS streaming is experimental)
...
# Create an instance of the EchoTTSWrapper class and server
app = FastAPI()
XTTS = EchoTTSWrapper(OUTPUT_FOLDER,SPEAKER_FOLDER,MODEL_FOLDER,LOWVRAM_MODE,MODEL_SOURCE,MODEL_VERSION,DEVICE,DEEPSPEED,USE_CACHE)

# Load model
logger.info(f"Loading Echo TTS model (version: {MODEL_VERSION})...")
logger.info("This may take 1-2 minutes on first run (models will be cached from HuggingFace).")
...
if STREAM_MODE or STREAM_MODE_IMPROVE:
    ...
    logger.warning("For best results, use non-streaming mode.")
...
async def tts_stream(request: Request, text: str = Query(), speaker_wav: str = Query(), language: str = Query()):
    ...
    # Warn about experimental streaming
    logger.warning("Streaming mode with Echo TTS is experimental. Full audio is generated then streamed in chunks.")
